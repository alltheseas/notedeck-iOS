...
extension InputEvent {
    ...
    class public func from_paste<GenericIntoRustString: IntoRustString>(_ text: GenericIntoRustString) -> InputEvent {
        InputEvent(ptr: __swift_bridge__$InputEvent$from_paste({ let rustString = text.intoRustString(); rustString.isOwned = false; return rustString.ptr }()))
    }
}
...
extension InputEventRef {
    class public func from_copy() -> InputEvent {
        InputEvent(ptr: __swift_bridge__$InputEvent$from_copy())
    }

    class public func from_cut() -> InputEvent {
        InputEvent(ptr: __swift_bridge__$InputEvent$from_cut())
    }
}
...
extension OutputStateRef {
    ...
    public func get_copied_text() -> RustStr {
        __swift_bridge__$OutputState$get_copied_text(ptr)
    }
}
...
public class NotedeckRenderer: NotedeckRendererRefMut {
    var isOwned: Bool = true
    ...
    deinit {
        if isOwned {
            __swift_bridge__$NotedeckRenderer$_free(ptr)
        }
    }
}
extension NotedeckRenderer {
    public convenience init<GenericIntoRustString: IntoRustString>(_ layer_ptr: UnsafeMutableRawPointer, _ width: UInt32, _ height: UInt32, _ display_scale: Float, _ data_path: GenericIntoRustString) {
        self.init(ptr: __swift_bridge__$NotedeckRenderer$new(layer_ptr, width, height, display_scale, { let rustString = data_path.intoRustString(); rustString.isOwned = false; return rustString.ptr }()))
    }
}
public class NotedeckRendererRefMut: NotedeckRendererRef {
    public override init(ptr: UnsafeMutableRawPointer) {
        super.init(ptr: ptr)
    }
}
extension NotedeckRendererRefMut {
    public func resize(_ width: UInt32, _ height: UInt32) {
        __swift_bridge__$NotedeckRenderer$resize(ptr, width, height)
    }

    public func set_safe_area(_ top: Float, _ right: Float, _ bottom: Float, _ left: Float) {
        __swift_bridge__$NotedeckRenderer$set_safe_area(ptr, top, right, bottom, left)
    }

    public func render(_ time: Double, _ input_events: RustVec<InputEvent>) -> OutputState {
        OutputState(ptr: __swift_bridge__$NotedeckRenderer$render(ptr, time, { let val = input_events; val.isOwned = false; return val.ptr }()))
    }
}
public class NotedeckRendererRef {
    var ptr: UnsafeMutableRawPointer
...
extension NotedeckRenderer: Vectorizable {
    public static func vecOfSelfNew() -> UnsafeMutableRawPointer {
        __swift_bridge__$Vec_NotedeckRenderer$new()
    }

    public static func vecOfSelfFree(vecPtr: UnsafeMutableRawPointer) {
        __swift_bridge__$Vec_NotedeckRenderer$drop(vecPtr)
    }

    public static func vecOfSelfPush(vecPtr: UnsafeMutableRawPointer, value: NotedeckRenderer) {
        __swift_bridge__$Vec_NotedeckRenderer$push(vecPtr, {value.isOwned = false; return value.ptr;}())
    }

    public static func vecOfSelfPop(vecPtr: UnsafeMutableRawPointer) -> Optional<Self> {
        let pointer = __swift_bridge__$Vec_NotedeckRenderer$pop(vecPtr)
        if pointer == nil {
            return nil
        } else {
            return (NotedeckRenderer(ptr: pointer!) as! Self)
        }
    }

    public static func vecOfSelfGet(vecPtr: UnsafeMutableRawPointer, index: UInt) -> Optional<NotedeckRendererRef> {
        let pointer = __swift_bridge__$Vec_NotedeckRenderer$get(vecPtr, index)
        if pointer == nil {
            return nil
        } else {
            return NotedeckRendererRef(ptr: pointer!)
        }
    }

    public static func vecOfSelfGetMut(vecPtr: UnsafeMutableRawPointer, index: UInt) -> Optional<NotedeckRendererRefMut> {
        let pointer = __swift_bridge__$Vec_NotedeckRenderer$get_mut(vecPtr, index)
        if pointer == nil {
            return nil
        } else {
            return NotedeckRendererRefMut(ptr: pointer!)
        }
    }

    public static func vecOfSelfAsPtr(vecPtr: UnsafeMutableRawPointer) -> UnsafePointer<NotedeckRendererRef> {
        UnsafePointer<NotedeckRendererRef>(OpaquePointer(__swift_bridge__$Vec_NotedeckRenderer$as_ptr(vecPtr)))
    }

    public static func vecOfSelfLen(vecPtr: UnsafeMutableRawPointer) -> UInt {
        __swift_bridge__$Vec_NotedeckRenderer$len(vecPtr)
    }
}
